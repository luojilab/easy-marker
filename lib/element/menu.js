...
export default class Menu extends BaseElement {
  ...
  createElement() {
    ...
    this.option.items.forEach((item) => {
      const menuItem = this.createMenuItemElement(item.text, item.iconClassList, item.style)
  ...
  createMenuItemElement(text, iconClassList, itemStyle) {
    // eslint-disable-line class-methods-use-this
    const menuItem = document.createElement('span')
    Object.assign(menuItem.style, this.option.style.item, itemStyle)
    if (iconClassList) {
      const iconItem = document.createElement('span')
      Object.assign(iconItem.style, { display: 'block' })
      iconClassList.forEach((item) => {
        iconItem.classList.add(item)
      })
  ...
  handleTap(e, {
    ...
    if (item.id && this.option.handler) {
      this.option.handler.call(this.easyMarker, item.id, selection)
    } else {
  ...
  getTapTarget(target) {
    if (this.itemMap.has(target) || target.classList.contains('em-menu')) {
      return target
    }
