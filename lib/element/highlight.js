...
export default class Highlight extends BaseElement {
  ...
  highlight(selection, id, meta = {}, offset) {
    ...
    const lineHeight = Number(window.getComputedStyle(selection.anchorNode.parentElement).lineHeight.replace('px', ''))
    ...
    this.lineMap.set(lineID, {
      selection, points, rects, meta, lineHeight,
    })
    return lineID
  }
  ...
  handleTap(e) {
    ...
    this.lineMap.forEach((line, id) => {
      for (let i = 0; i < line.rects.length; i++) {
        ...
        if (inRectangle(x, y, rect, margin)) {
          clickLine = { id, line }
          break
        }
      }
