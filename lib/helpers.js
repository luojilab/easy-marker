...
export function getClickWordsPosition(pElement, x, y, separators = ['']) {
  if (!pElement || (!pElement && pElement.childNodes)) return {}
  const lineHeight = Number(window.getComputedStyle(pElement).lineHeight.replace('px', ''))
    ...
    if (node.nodeName === '#text') {
      ...
      for (let index = 0; index < words.length; index++) {
        ...
        if (rect.top - margin <= y && rect.bottom + margin >= y) {
          ...
          if (x < rect.left || (rect.left < x && rect.right > x)) {
            const isLineStart = lastRect === null || (lastRect && lastRect.top !== rect.top)
            ...
            if ((!isLeft || (isLineStart && !isStart)) && !(isLineEnd && isStart)) {
          ...
          const isLeft = position.x === lastRect.left
          if (isStart && !isLeft) {
            position.x -= lastRect.width
            position.index -= 1
          } else if (!isStart && isLeft) {
            position.x += lastRect.width
            position.index += 1
          }
          break
 ...
 * @returns {boolean}
 */
export function inRectangle(x, y, rect) {
