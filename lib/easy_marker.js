...
class EasyMarker {
   ...
   * @param {string} options.scrollSpeedLevel The speed of scrolling when touching bottom, default 4
   * @param {number|string} options.scrollOffsetBottom The distance from the bottom when triggering scrolling，default 100
   * @param {Object} options.markdownOption markdown option
   * @param {Boolean} options.markdownOption.isReplaceAll Whether to directly replace the default configuration, default false
   * @param {Function} options.markdownOption.generalWrapMarkdown Universal method for handling Markdown
   * @param {Object} options.markdownOption.options Mapping relations between HTML and Markdown
   * @memberof EasyMarker
   */
  constructor(options) {
  ...
  create(containerElement, scrollContainerElement, excludeElements = []) {
    ...
    if (this.options.cursor.same) {
      this.cursor.start = new Cursor(this.container, CursorType.END)
    } else {
    ...
    this.markdown = new Markdown(this.container, this.options.markdownOption)
