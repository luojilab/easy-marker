...
class EasyMarker {
  ...
  create(containerElement, scrollContainerElement, excludeElements = []) {
    ...
    const CursorElement = this.options.cursor && this.options.cursor.Cursor
      ? this.options.cursor.Cursor : Cursor

    if (this.options.cursor.same) {
      this.cursor.start = new CursorElement(this.container, CursorType.END, this.options.cursor || {})
    } else {
      this.cursor.start = new CursorElement(this.container, CursorType.START, this.options.cursor || {})
    }
