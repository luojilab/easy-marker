import { getNodeRects } from './helpers'
  ...
  static getSelectRects(startTextNode, endTextNode) {
    ...
    rects.forEach((rect) => {
      if (lineMergedRectMap.has(rect.top)) {
        ...
        lineMergedRectMap.set(rect.top, rect)
      }
    })
    ...
    if (lineMergedRects.length > 0) {
      startLineHeight = Number(window.getComputedStyle(startTextNode.node.parentElement).lineHeight.replace('px', ''))
    ...
    if (lineMergedRects.length > 0) {
      ...
      endLineHight = Number(window.getComputedStyle(endTextNode.node.parentElement).lineHeight.replace('px', ''))
